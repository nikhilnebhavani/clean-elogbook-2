...
export default function ManageCompetency({ token }) {
  ...
  const handleLogSubmit = async (e) => {
    ...
    if (add == 1 && (course.id == 0 || subject.id == 0 || competency == "")) {
      ...
      add == 2 &&
      (course.id == 0 || subject.id == 0 || fileData.length <= 0)
    ) {
      setError("fill all mandatory fields");
      return;
    }
    ...
    if (update == "") {
      try {
        ...
        const add_log = await fetch(`${backend_url}/addTCompetency`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        setPopup({
          ...
          message:
            add == 1
              ? "Competency"
              : "Competencies" + "adding failed. Please try again.",
        });
      } finally {
        setLoading(false);
      }
      ...
      try {
        ...
        const add_log = await fetch(`${backend_url}/updateTCompetency`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        if (add_log.ok) {
          ...
          setPopup({
            ...
            type: "error",
            message:
              data.error || "Competencyupdate failed.",
          });
          setLoading(false);
        }
      } catch (error) {
        setPopup({
          active: true,
          type: "error",
          message:"Competency update failed. Please try again.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleSearch = (e) => {
    ...
    setFilterCompetencies(
      competencies.filter((e) => e.subject_id == subject.id)
    );
  };
  ...
  const handleActive = async (id, active) => {
    ...
    try {
      ...
      const add_log = await fetch(`${backend_url}/activeTCompetency`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (add_log.ok) {
        setPopup({
          active: true,
          type: "success",
          message: `Competency ${active ? "Active" : "Inactive"} successfully.`,
        });
        setLoading(false);
      } else {
        setPopup({
          active: true,
          type: "error",
          message: data.error || "Competency update failed.",
        });
        setLoading(false);
      }
    } catch (error) {
                  ...
                  <tr key={rowIndex}>
                    ...
                    <td className={styles.tableCell}>
                      <button
                        ...
                        onClick={() => {
                          handleActive(competency._id, !competency.active);
                        }}
                      >
