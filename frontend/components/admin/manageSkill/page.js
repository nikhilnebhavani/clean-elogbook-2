...
export default function ManageSkill({token}) {
  ...
  const handleLogSubmit = async (e) => {
    ...
    try {
      ...
      const add_log = await fetch(`${backend_url}/addASkill`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    //   if (
    //     add == 1 &&
    //     (course.id == 0 || subject.id == 0 || competency.id == 0 || skill == "")
    //   ) {
    //     setError("fill all mandatory fields");
    //     return;
    //   } else if (
    ...
    //     const add_log = await fetch(`${backend_url}/bulkAAddSkills`, {
    //       method: "POST",
    //       headers: {
    //         "Content-Type": "application/json",
    //         Authorization: `Bearer ${token}`,
  ...
  //     setError("Fill all mandatory fields");
  //     return;
  //   } else if (add == 2 && fileData.length <= 0) {
  ...
  //       // If any failed entries, download as XLSX
  //       if (data.failed && data.failed.length > 0) {
  //         const XLSX = await import("xlsx");

  //         const worksheet = XLSX.utils.json_to_sheet(data.failed);
  //         const workbook = XLSX.utils.book_new();
  //         XLSX.utils.book_append_sheet(workbook, worksheet, "Failed Skills");
  ...
  const toggleActive = async (skill) => {
    ...
    try {
      ...
      console.log("Skill ID type:", typeof skill.skill_id);
      
      const newActiveState = !skill.active;
      ...
      const response = await fetch(`${backend_url}/updateASkill`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      setPopup({
        active: true,
        type: "success",
        message: `Skill ${skill.active ? "deactivated" : "activated"} successfully`,
      });
    } catch (error) {
      ...
      setPopup({
        active: true,
        type: "error",
        message: "Failed to update skill status: " + error.message,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleEditClick = (skill) => {
    ...
    setCourse({
      title: subjects.find(s => s.subject_id === skill.subject_id)?.course_name || "",
      id: subjects.find(s => s.subject_id === skill.subject_id)?.course_id || 0
    });
    setSubject({
      title: subjects.find(s => s.subject_id === skill.subject_id)?.subject_name || "",
      id: skill.subject_id
    });
    setCompetency({
      title: competencies.find(c => c.competency_id === skill.competency_id)?.competency_name || "",
                ...
                filterskills.map((skill, rowIndex) => (
                  <tr key={rowIndex}>
                    ...
                    <td className={styles.tableCell}>
                      {
                        subjects.find((e) => parseInt(e.subject_id) === parseInt(skill.subject_id))?.subject_name || "Unknown Subject"
                      }
                    </td>
                    <td className={styles.tableCell}>
                      {
                        competencies.find((e) => e.competency_id === skill.competency_id)?.competency_name || "Unknown Competency"
                      }
                    </td>
                    <td className={styles.tableCell}>{skill.skill_name}</td>
                    <td className={styles.tableCell}>
                      {skill.created_at ? new Date(skill.created_at).toLocaleString() : "N/A"}
                    </td>
                    <td className={styles.tableCell}>
                      {skill.updated_at ? new Date(skill.updated_at).toLocaleString() : "N/A"}
                    </td>
                    <td className={styles.tableCell}>
                      <div className={styles.actionButtons}>
                        ...
                        <button
                          ...
                          className={skill.active ? styles.inactiveButton : styles.activeButton}
                        >
